...
/**
 * Tiles use the editor's Sanity upload when present (already square-cropped
 * around the hotspot) and fall back to the built-in photo in /public.
 */
export default function CategoryGrid({
  images = {},
}: {
  images?: SiteSettingsData["categoryImages"];
}) {
  return (
    <Section>
      <Container wide>
        ...
        <Stagger className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5 md:gap-5">
          {CARDS.map((card) => {
            const custom = images[card.slug as keyof typeof images];
            return (
              <StaggerItem key={card.slug}>
                ...
                >
                  <div className="relative aspect-square overflow-hidden">
                    <Image
                      src={custom ?? card.image}
                      alt={card.title}
                      fill
                      sizes="(min-width: 1024px) 20vw, (min-width: 768px) 33vw, 50vw"
                      className={`object-cover transition-transform duration-700 ease-out group-hover:scale-[1.05] ${custom ? "" : (card.imagePosition ?? "")}`}
                    />
                  </div>
